...
6. **ErrorResponseSerializer**. Вызывается, если при пост-валидации ответа контроллера произошла ошибка.
7. **Serializer**. Сериализует данные, пришедшие из пост-валидатора, и возвращает их пользователю. Здесь данные могут запихиваться в django templates, json, xml, yaml или другие форматы ответа.

Обязательными являются только Controller и Serializer.
...
* Валидаторы:
	* Первый валидирует пришедшие данные, второй - ответ контроллера
	* Если указан только один валидатор, то он будет использоваться для валидации пришедших данных.
	* Если нужно указать только пост-валидатор, то вместо валидатора можно передать `None`.
* Сериализаторы:
	* Сериализаторы также указываются в порядке вызова: ErrorSerializer, ErrorResponseSerializer, Serializer
	* Если указан один, то он будет использоваться как ErrorSerializer, ErrorResponseSerializer, Serializer
	* Если указано два, то первый ErrorSerializer, ErrorResponseSerializer, а второй - Serializer

Можно использовать и более строгое определение с помощью класса `Rule`:
...
Рекомендации:
* Рекомендуемый способ описания правил -- первый, т.к. второй сложнее запомнить и дольше писать, а третий плохо читается.
...
### Serializer, ErrorSerializer и ErrorResponseSerializer
