...
2. **Parser** (`p`). Получает на вход запрос и достаёт из него данные. По умолчанию достает либо данные из строки GET-запроса, либо тело для остальных запросов.
3. **PreValidator** (`prev`). Производит валидацию приходящих данных. Основная логика в принципе не может возвращать ошибки, только поднимать исключения, которые уже будут обрабатываться самим Django, вне DjBurger. В качестве валидаторов используются немного пропатченные django-формы. Также валидатор очищает данные, которые уже попадают в контроллер:
